...
def swine_align(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4a
    true_gcd, possible_gcd = 1, 1
    while possible_gcd <= player_score or possible_gcd <= opponent_score:
        if player_score % possible_gcd == 0 and opponent_score % possible_gcd == 0:
            true_gcd = possible_gcd
        possible_gcd += 1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who == 0:
            point_gain = score0 - last_score
            latest_score = score0
        elif who == 1:
            point_gain = score1 - last_score
            latest_score = score1
        if point_gain > running_high:
            print(point_gain, "point(s)! The most yet for Player", who)
            highest = point_gain
        return announce_highest(who, latest_score, highest)
    return say
    # END PROBLEM 7
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    score_if_zero = free_bacon(opponent_score) + score
    if extra_turn(score_if_zero, opponent_score):
        return 0
    else:
